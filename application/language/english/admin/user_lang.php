<?php
$lang['user_info']							= 'Quản lý khách hàng';//'Quản lý thành viên';

$lang['title_verify_info']					= 'Thông tin xác thực';
$lang['title_verify_edit']					= 'Chỉnh sửa thông tin xác thực';

$lang['password_repeat']					= 'Nhập lại mật khẩu';
$lang['user_is_blocked']					= 'Tài khoản này đã bị khóa';
$lang['user_unblock']						= 'Đang hoạt động';
$lang['user_blocked']						= 'Bị khóa';
$lang['block_user']				        	= 'Khóa tài khoản';
$lang['unblock_user']				        = 'Mở lại tài khoản';
$lang['user_date_added']					= 'Thành viên từ';
$lang['admin_login_user']					= 'Đăng nhập vào tài khoản khách hàng';//'Đăng nhập vào tài khoản thành viên';
$lang['verify']								= 'Xác thực';
$lang['view_verify_info']					= 'Xem thông tin xác thực';
$lang['blocked']							= 'Khóa';
$lang['balance']							= 'Số dư';
$lang['transaction']						= 'Giao dịch';
$lang['withdraw']							= 'Rút tiền';
$lang['deposit']							= 'Nạp tiền';

$lang['affiliate']                          = 'Affiliate';
$lang['affiliate_key']                      = 'Mã giới thiệu';

$lang['user_group']							= 'Nhóm khách hàng';//'Nhóm thành viên';
$lang['payment']				        	= 'Cổng thanh toán';
$lang['payment_total_amount']				= 'Số tiền giao dịch tối đa trong 1 ngày';

$lang['full_name']				   			= 'Họ tên';
$lang['card_no']				   			= 'Số chứng minh nhân dân';
$lang['card_place']				   			= 'Nơi cấp';
$lang['card_date']				   			= 'Ngày cấp';
$lang['paypal_emails']				   		= 'Các email sử dụng khi thanh toán qua paypal';
$lang['image_card_front']				   	= 'Mặt trước CMND/Hộ chiếu';
$lang['image_card_back']				   	= 'Mặt sau CMND/Hộ chiếu';
$lang['image_photo']				   		= 'Ảnh cá nhân';

$lang['discount']				        	= 'Hoa hống giới thiệu';

$lang['select_action']				   		= 'Hành động...';
$lang['action_block']						= 'Khóa tài khoản';
$lang['action_unblock']						= 'Mở lại tài khoản';
$lang['action_del']							= 'Xóa';

$lang['note_password']						= 'Mật khẩu phải có ít nhất 6 kí tự';
$lang['note_password_change']				= 'Khi muốn thay đổi mật khẩu thì mới nhập giá trị';
$lang['note_paypal_emails']					= 'Mỗi email một dòng';

$lang['notice_are_you_sure_want_to_delete']	= 'Bạn có chắc chắn muốn xóa khách hàng';//'Bạn có chắc chắn muốn xóa thành viên';
$lang['notice_are_you_sure_want_to_block']	= 'Bạn có chắc chắn muốn khóa tài khoản';
$lang['notice_are_you_sure_want_to_unblock']= 'Bạn có chắc chắn muốn mở lại tài khoản';
$lang['notice_verify_are_you_sure_want_to_accept']	= 'Bạn có chắc chắn muốn xác thực cho khách hàng: %s ?
												Khi xác thực thì hệ thống sẽ tự động gán khách hàng vào nhóm: %s.
											';//'Bạn có chắc chắn muốn xác thực cho thành viên: %s ?
												//Khi xác thực thì hệ thống sẽ tự động gán thành viên vào nhóm: %s.
											//';
$lang['notice_verify_are_you_sure_want_to_cancel']	= 'Bạn có chắc chắn muốn hủy bỏ thông tin xác thực của khách hàng: %s ?
												Khi hủy bỏ thì hệ thống sẽ tự động gán khách hàng vào nhóm: %s.
											';//'Bạn có chắc chắn muốn hủy bỏ thông tin xác thực của thành viên: %s ?
												//Khi hủy bỏ thì hệ thống sẽ tự động gán thành viên vào nhóm: %s.
											//';

?>
