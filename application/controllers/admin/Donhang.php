...
*/
	   /*if($this->form_validation->run())
	   {
		...
		if ($this->input->post('donhang_add')) {
			//lay du lieu tu form
      $cap_bac = $this->input->post('cap_bac');
        ...
        'table_buyer' => $cap_bac,
				...
				'buyer_id'     => $user_id,
				'ngay_giao_dich'    => $ngay_dat_hang,
				'sub_id' => $user_refer_id,
				//'created'  => now()
			);
			...
			if($this->donhang_model->create($data))
			{
				...
				$donhang_n     = $this->input->post('bao_nhieu_mat_hang'); //echo $dathang_n; die();
				...
				for ($i = 0; $i < $donhang_n; $i++) {
					...
					$this->donhang_dong_model->create($data);

				}

				redirect(base_url('admin/donhang'));
    ...
    function index()
    {
        ...
        $config['base_url']   = admin_url('donhang/index'); //link hien thi ra danh sach giao dich
		...
		foreach ($list as $row) {
			...
			$query = $this->db->query('SELECT SUM(soluong * giadonvi) as tongthanhtien FROM donhang_dong WHERE donhang_id = ' . $donhang_id);
