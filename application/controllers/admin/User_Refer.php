 ...
 */
	...
	function add()
	{
	   ...
	   if($this->form_validation->run())
	   {
		...
		// Hien thi view
		$this->data['temp'] = 'admin/user_refer/add';
	...
	function edit()
	{
	   ...
	   if($this->form_validation->run())
	   {
	       ...
	       //them thanh vien vao trong csdl
	       if($this->user_refer_model->update($id, $data))
	       {
	       	  $this->session->set_flashdata('flash_message', 'Cập nhật thành viên thành công');
			  redirect(base_url('admin/user_refer'));//chuyen toi trang danh sách thành viên
	       }
	    }
		$this->data['action'] = current_url();
		$this->data['info']   = $info;
		...
		// Hien thi view
		$this->data['temp'] = 'admin/user_refer/edit';
		$this->load->view('admin/main', $this->data);
	}
	...
	{
	    $user = $this->user_refer_model->get_info($id);
	    if(!$user)
	    {
	        //tạo ra nội dung thông báo
	        $this->session->set_flashdata('message', 'không tồn tại thành viên này');
	    ...
	    $this->user_refer_model->delete($id);
 ...
 */
	...
	function index()
	{
	     //Buoc 1:load thu vien phan trang
   	    ...
   	    $total_rows = $this->user_refer_model->get_total();
   	    $config = array();
   	    $config['base_url']    = base_url('admin/user_refer/index');
		...
		// Hien thi view
		$this->data['temp'] = 'admin/user_refer/index';
		$this->load->view('admin/main', $this->data);
	}
