...
class Taotaikhoan extends MY_Controller {
   ...
   function taosub()
	{
	   ...
	   if($this->form_validation->run())
	   {
	       ...
	       if($this->sub_model->create($data))
	       {
	       	  $this->session->set_flashdata('flash_message', 'Thêm thành viên thành công');
			  redirect(base_url('admin/taotaikhoan/indexsub'));//chuyen toi trang danh sách thành viên
	       }
	    }
		...
		// Hien thi view
		$this->data['temp'] = 'admin/taotaikhoan/taosub';
 ...
 */
	...
	function taotongdaily()
	{
	   ...
	   if($this->form_validation->run())
	   {
	       ...
	       {
	       	  $this->session->set_flashdata('flash_message', 'Thêm thành viên thành công');
			  redirect(base_url('admin/taotaikhoan/indextongdaily'));//chuyen toi trang danh sách thành viên
	       }
	    }
		...
		// Hien thi view
		$this->data['temp'] = 'admin/taotaikhoan/taotongdaily';
		$this->load->view('admin/main', $this->data);
	}
	...
	function indexsub()
	{
	     //Buoc 1:load thu vien phan trang
   	    ...
   	    $total_rows = $this->sub_model->get_total();

   	    $this->data['total_rows'] = $total_rows;

   	    $config = array();
   	    $config['base_url']    = base_url('admin/taotaikhoan/indexsub');
		...
		$list = array();
		$list = $this->sub_model->get_list($input);
		$this->data['list'] = $list;
		...
		// Hien thi view
		$this->data['temp'] = 'admin/taotaikhoan/indexsub';
		$this->load->view('admin/main', $this->data);
	}

	function indextongdaily()
	{
	     //Buoc 1:load thu vien phan trang
   	    $this->load->library('pagination');
   	    //Buoc 2:Cau hinh cho phan trang
   	    //lay tong so luong thanh vien tu trong csdl
   	    $total_rows = $this->tongdaily_model->get_total();

   	    $this->data['total_rows'] = $total_rows;

   	    $config = array();
   	    $config['base_url']    = base_url('admin/taotaikhoan/indextongdaily');
   	    $config['total_rows']  = $total_rows;
   	    $config['per_page']    = 10;
   	    $config['uri_segment'] = 4;//phân đoạn 4
   	    $config['next_link']   = "Trang kế tiếp";
		...
		// Hien thi view
		$this->data['temp'] = 'admin/taotaikhoan/indextongdaily';
		$this->load->view('admin/main', $this->data);
	}
 ...
 */
	...
	function index()
	{
	     //Buoc 1:load thu vien phan trang
   	    ...
   	    $total_rows = $this->tongdaily_model->get_total();
   	    $config = array();
   	    $config['base_url']    = base_url('admin/taotaikhoan/index');
   	    $config['total_rows']  = $total_rows;
   	    $config['per_page']    = 10;
   	    $config['uri_segment'] = 4;//phân đoạn 4
   	    $config['next_link']   = "Trang kế tiếp";
